...
def _automatic_duration_expr(now_dt: datetime, from_dt: datetime | None = None, to_dt: datetime | None = None):
...
def _source_sums(db: Session, guild_id: int, from_dt: datetime | None = None, to_dt: datetime | None = None):
    ...
    automatic = (
        db.query(func.coalesce(func.sum(_automatic_duration_expr(now_dt, from_dt, to_dt)), 0))
...
def get_overview(db: Session, guild_id: int, from_dt: datetime | None = None, to_dt: datetime | None = None) -> dict:
    ...
    games = (
        ...
        .filter(
            ...
            | (db.query(PlaytimeAdjustment.id).filter(PlaytimeAdjustment.guild_id == guild_id, PlaytimeAdjustment.game_id == Game.id).exists()),
        )
        .scalar()
        or 0
    )
...
def ranked_games(
    ...
    auto_q = (
        db.query(
            ...
            func.coalesce(func.sum(_automatic_duration_expr(now_dt, from_dt, to_dt)), 0).label("auto_seconds"),
        )
        .filter(ActivitySession.guild_id == guild_id, *_automatic_overlap_filters(now_dt, from_dt, to_dt))
